...
GPUEngine::GPUEngine(int nbThreadGroup, int nbThreadPerGroup, int gpuId, uint32_t maxFound,bool rekey,bool needsRekey) {

  // Initialise CUDA
  this->rekey = rekey;
  this->needsRekey = needsRekey;
...
bool GPUEngine::SetKeys(Point *p) {
  ...
  // Проверяем, есть ли pinned память; если нет — выделяем заново
  if (inputKeyPinned == NULL) {
    cudaError_t err = cudaHostAlloc(&inputKeyPinned, nbThread * 32 * 2, cudaHostAllocWriteCombined | cudaHostAllocMapped);
    if (err != cudaSuccess) {
      printf("GPUEngine: SetKeys realloc pinned memory: %s\n", cudaGetErrorString(err));
      return false;
    }
  }
...
// Быстрое обновление ключей (без выделения памяти)
bool GPUEngine::Rekey(Point *p) {

  if (inputKeyPinned == NULL) {
    // Если pinned память была освобождена — выделяем заново
    cudaError_t err = cudaHostAlloc(&inputKeyPinned, nbThread * 32 * 2, cudaHostAllocWriteCombined | cudaHostAllocMapped);
    if (err != cudaSuccess) {
      printf("GPUEngine: Rekey alloc pinned memory: %s\n", cudaGetErrorString(err));
      return false;
    }
  }
