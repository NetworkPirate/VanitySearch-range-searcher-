...
public:
  ...
  GPUEngine(int nbThreadGroup,int nbThreadPerGroup,int gpuId,uint32_t maxFound,bool rekey,bool needsRekey=false);
  ~GPUEngine();
  ...
  bool SetKeysRange(Point *startP, uint64_t keysPerThread);  // Для range search
  void SetSearchMode(int searchMode);
  void SetSearchType(int searchType);
  void SetPattern(const char *pattern);
  bool Launch(std::vector<ITEM> &prefixFound,bool spinWait=false);
  bool Rekey(Point *p);  // Только обновление ключей без выделения памяти
  bool RekeyStep(Point *startP, uint64_t step);  // Обновление: начальная точка + шаг
  int GetNbThread();
  int GetGroupSize();
...
private:
  ...
  bool lostWarning;
  bool rekey;
  bool needsRekey;  // Флаг: нужен ли повторный вызов SetKeys (для range search)
  uint32_t maxFound;
  uint32_t outputSize;
  std::string pattern;
  bool hasPattern;
