...
VanitySearch::VanitySearch(Secp256K1 *secp, vector<std::string> &inputPrefixes,string seed,int searchMode,
                           ...
                           string startKeyHex, string endKeyHex)
  ...
  // Seed
  useDirectStartKey = false;
  useRangeSearch = false;
  endKey.SetInt32(0);
  logFile = NULL;

  if (!startKeyHex.empty()) {
    // Прямое указание startKey в hex формате
    startKey.SetBase16((char *)startKeyHex.c_str());
    useDirectStartKey = true;
    printf("Using direct start key: %s\n", startKey.GetBase16().c_str());
  } else {
    // Стандартная генерация из seed
    if (seed.length() == 0) {
      // Default seed
  ...
  // Установка конечной точки диапазона
  if (!endKeyHex.empty()) {
    endKey.SetBase16((char *)endKeyHex.c_str());
    useRangeSearch = true;
    printf("Search range: %s -> %s\n", startKey.GetBase16().c_str(), endKey.GetBase16().c_str());
    
    // Проверка валидности диапазона
    if (endKey.IsLowerOrEqual(&startKey)) {
      printf("Warning: endKey <= startKey! Range search disabled.\n");
      useRangeSearch = false;
    }
  }
...
void VanitySearch::getCPUStartingKey(int thId,Int& key,Point& startP) {
  ...
  if (rekey > 0) {
    // Rekey отключает range search — ключи случайные
    key.Rand(256);
  } else if (useRangeSearch) {
    // Для range search: потоки идут последовательно без пропусков
    // Поток i начинает с: startKey + i * CPU_GRP_SIZE
    key.Set(&startKey);
    Int offset((uint64_t)thId);
    Int grpSize((uint64_t)CPU_GRP_SIZE);
    offset.Mult(&grpSize);  // offset = thId * CPU_GRP_SIZE
    key.Add(&offset);
  } else {
    key.Set(&startKey);
...
void VanitySearch::FindKeyCPU(TH_PARAM *ph) {
  ...
  while (!endOfSearch) {
    ...
    // Проверка достижения конца диапазона
    if (useRangeSearch && key.IsGreaterOrEqual(&endKey)) {
      if (logFile) {
        fprintf(logFile, "[CPU Thread %d] End of range reached (key=%s >= endKey=%s)\n",
               thId, key.GetBase16().c_str(), endKey.GetBase16().c_str());
        fflush(logFile);
      }
      endOfSearch = true;
      break;
...
void VanitySearch::getGPUStartingKeys(int thId, int groupSize, int nbThread, Int *keys, Point *p) {
  ...
  if (useRangeSearch) {
    // Для range search: потоки идут последовательно без пропусков
    // Поток i начинает с: startKey + i * groupSize
    Int grpSize((uint64_t)groupSize);
    for (int i = 0; i < nbThread; i++) {
      keys[i].Set(&startKey);
      Int offset((uint64_t)i);
      offset.Mult(&grpSize);  // offset = i * groupSize
      keys[i].Add(&offset);
    ...
    // Стандартное распределение с большими смещениями
    for (int i = 0; i < nbThread; i++) {
      if (rekey > 0) {
        keys[i].Rand(256);
      } else {
...
void VanitySearch::FindKeyGPU(TH_PARAM *ph) {
  ...
  // Global init
  int thId = ph->threadId;
  GPUEngine g(ph->gridSizeX,ph->gridSizeY, ph->gpuId, maxFound, (rekey!=0), useRangeSearch);
  ...
  while (ok && !endOfSearch) {
    ...
    // Проверка достижения конца диапазона
    if (useRangeSearch) {
      int lastThreadIdx = nbThread - 1;
      
      if (keys[lastThreadIdx].IsGreaterOrEqual(&endKey)) {
        fprintf(logFile, "[GPU] End of range reached (last thread %d: %s >= %s)\n",
               lastThreadIdx, keys[lastThreadIdx].GetBase16().c_str(), endKey.GetBase16().c_str());
        fflush(logFile);
        endOfSearch = true;
        break;
      }
    ...
    if (ok) {
      if (useRangeSearch) {
        // При range search: ВСЕ потоки сдвигаются на общее количество ключей за итерацию
        // Чтобы не было перекрытия между итерациями
        Int step((uint64_t)nbThread);
        Int grpSize((uint64_t)g.GetGroupSize());
        step.Mult(&grpSize);  // step = nbThread * groupSize
        
        // Вычисляем step × G — публичную точку сдвига
        static Point stepG;
        static bool stepGComputed = false;
        static Int prevStep;
        
        if (!stepGComputed || !step.IsEqual(&prevStep)) {
          Int stepInt(step);
          stepG = secp->ComputePublicKey(&stepInt);
          prevStep.Set(&step);
          stepGComputed = true;
        }
        
        // Обновляем публичные ключи через сложение точек
        for (int i = 0; i < nbThread; i++) {
          keys[i].Add(&step);
          p[i] = secp->AddDirect(p[i], stepG);
        }
        counters[thId] += 6ULL * STEP_SIZE * nbThread;
        
        // Используем Rekey вместо SetKeys (быстрее, без аллокаций)
        ok = g.Rekey(p);
        
        if (!ok) {
          fprintf(logFile, "[GPU ERROR] Rekey failed at iter %d!\n", gpuIterCount);
          fflush(logFile);
        }
        
        // Прогресс каждые 100 итераций
        if (gpuIterCount % 100 == 0 && logFile) {
          fprintf(logFile, "[GPU Progress] Iter %d: first=%s, last=%s, end=%s\n",
                 gpuIterCount, keys[0].GetBase16().c_str(), 
                 keys[nbThread-1].GetBase16().c_str(), endKey.GetBase16().c_str());
          fflush(logFile);
        }
      } else {
...
void VanitySearch::Search(int nbThread,std::vector<int> gpuId,std::vector<int> gridSize) {
  ...
  if (useRangeSearch) {
    // Создаём лог рядом с exe файлом
    char logPath[MAX_PATH];
    GetModuleFileNameA(NULL, logPath, MAX_PATH);
    char *lastSlash = strrchr(logPath, '\\');
    if (lastSlash) *(lastSlash + 1) = '\0';
    strcat(logPath, "vanitysearch_range.log");
    
    logFile = fopen(logPath, "w");
    if (logFile) {
      fprintf(logFile, "=== Range Search Log ===\n");
      fprintf(logFile, "Start: %s\n", startKey.GetBase16().c_str());
      fprintf(logFile, "End:   %s\n", endKey.GetBase16().c_str());
      fflush(logFile);
    }
  }
  ...
  while (isAlive(params)) {
    ...
    if (isAlive(params)) {
      if (useRangeSearch) {
        // Для range search: показываем прогресс и ожидаемое время
        Int rangeSize(&endKey);
        rangeSize.Sub(&startKey);
        double totalRange = rangeSize.ToDouble();
        double progress = (double)count / totalRange;
        if (progress > 1.0) progress = 1.0;
        
        double remaining = totalRange - count;
        double etaSeconds = remaining / avgKeyRate;
        
        // Формат: [Speed][GPU Speed][Progress XX.XX%][ETA][Found]
        if (etaSeconds < 0) etaSeconds = 0;
        int etaH = (int)(etaSeconds / 3600);
        int etaM = (int)(fmod(etaSeconds, 3600.0) / 60);
        int etaS = (int)fmod(etaSeconds, 60.0);
        
        printf("\r[%.2f Mkey/s][GPU %.2f Mkey/s][Progress: %.2f%%][ETA %02d:%02d:%02d][Found %d]  ",
          avgKeyRate / 1000000.0, avgGpuKeyRate / 1000000.0,
          progress * 100.0, etaH, etaM, etaS, nbFoundKey);
      } else {
  ...
  if (logFile) {
    fprintf(logFile, "\n=== Search completed ===\n");
    fclose(logFile);
    printf("Log saved to: vanitysearch_range.log\n");
  }
