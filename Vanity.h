...
public:
  ...
  VanitySearch(Secp256K1 *secp, std::vector<std::string> &prefix, std::string seed, int searchMode,
               ...
               bool caseSensitive,Point &startPubKey,bool paranoiacSeed,std::string startKeyHex="", std::string endKeyHex="");
...
private:
  ...
  Int beta2;
  Int lambda2;
  bool useDirectStartKey;  // Флаг: использовать ли прямой startKey
  Int endKey;              // Конечная точка диапазона (0 = бесконечно)
  bool useRangeSearch;     // Флаг: искать ли в диапазоне [startKey, endKey]
  FILE *logFile;           // Файл для логирования
